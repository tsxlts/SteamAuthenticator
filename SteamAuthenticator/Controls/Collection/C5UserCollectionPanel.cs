...
{
    internal class C5UserCollectionPanel : ItemCollectionPanel<C5UserPanel, C5Client>
    {
        public C5UserPanel SetOffer(C5Client client, int? offerCount)
        {
            if (client == null)
            {
                return null;
        ...
        protected override C5UserPanel CreateUserPanel(bool hasUser, C5Client client)
        {
            int index = ItemPanels.Count;
            int y = ItemSize.Height + 10;

            C5UserPanel panel = new C5UserPanel(hasUser)
            {
                Size = ItemSize,
